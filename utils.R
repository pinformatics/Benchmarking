...
pacman::p_load(tidyverse, stringdist, phonics, glue, caret, lubridate,
               ...
               scico, RColorBrewer, ggthemes,plotly, keras,vroom,
               janitor)
...
preprocess_data <- function(df, df_dob) {
  df %>%
    select(starts_with("id"),
           fname = first_name, lname = last_name,
           birth_age, gender_code, race_code,
           voter_reg_num, county_desc,
           name_suffix, res_street_address) %>%
    left_join(df_dob, by = c("voter_reg_num", "county_desc")) %>%
    mutate(
      birth_age = as.integer(birth_age),
      name_suffix = if_else(is.na(name_suffix), "", name_suffix),
      birth_year = year(dob)
    ) %>%
    mutate_if(is.character, str_to_lower) %>%
    add_count_name(fname, "ffreq", TRUE) %>%
    add_count_name(lname, "lfreq", TRUE) %>%
...
add_feature_vector <- function(df, df_a_mod, df_b_mod) {
  message("🚩 Check required columns...")
  required_cols <- c("fname", "lname", "dob", "gender_code", "race_code")
  stopifnot(all(required_cols %in% colnames(df_a_mod)))
  stopifnot(all(required_cols %in% colnames(df_b_mod)))
  
  message("🚩 Define safe_scale...")
  safe_scale <- function(x) {
    if (length(unique(x)) == 1 || sd(x, na.rm = TRUE) == 0) return(rep(0, length(x)))
    scale(x)[, 1]
  }
  
  message("🚩 Precompute name frequencies...")
  df_freq <- df %>% select(-contains("freq")) %>%
    left_join(df_a_mod %>% count(fname, name = "ffreq_a_raw") %>% rename(fname_a = fname) %>% mutate(ffreq_a = safe_scale(ffreq_a_raw))) %>%
    left_join(df_a_mod %>% count(lname, name = "lfreq_a_raw") %>% rename(lname_a = lname) %>% mutate(lfreq_a = safe_scale(lfreq_a_raw))) %>%
    left_join(df_b_mod %>% count(fname, name = "ffreq_b_raw") %>% rename(fname_b = fname) %>% mutate(ffreq_b = safe_scale(ffreq_b_raw))) %>%
    left_join(df_b_mod %>% count(lname, name = "lfreq_b_raw") %>% rename(lname_b = lname) %>% mutate(lfreq_b = safe_scale(lfreq_b_raw))) %>%
    mutate(across(starts_with("f") | starts_with("l"), ~ ifelse(is.na(.), 0, .)))
  
  message("🚩 Compute core features...")
  df_res <- df_freq %>%
    mutate(
      pair_id = row_number(),
      birth_year_a = as.character(lubridate::year(dob_a)),
      birth_year_b = as.character(lubridate::year(dob_b)),
      birth_day_a = as.character(lubridate::day(dob_a)),
      birth_day_b = as.character(lubridate::day(dob_b)),
      birth_month_a = as.character(lubridate::month(dob_a)),
      birth_month_b = as.character(lubridate::month(dob_b))
    )
  
  df_res$metric_dob_dist <- mapply(
    stringdist::stringdist,
    as.character(df_res$dob_a),
    as.character(df_res$dob_b),
    MoreArgs = list(method = "dl")
  )
  df_res$metric_year_dist <- mapply(stringdist::stringdist, df_res$birth_year_a, df_res$birth_year_b, MoreArgs = list(method = "dl"))
  df_res$metric_month_dist <- mapply(stringdist::stringdist, df_res$birth_month_a, df_res$birth_month_b, MoreArgs = list(method = "dl"))
  df_res$metric_day_dist <- mapply(stringdist::stringdist, df_res$birth_day_a, df_res$birth_day_b, MoreArgs = list(method = "dl"))
  
  df_res <- df_res %>%
    mutate(
      metric_dm_swaps = (!is.na(dob_a) & !is.na(dob_b)) & (birth_month_a == birth_day_b) & (birth_month_b == birth_day_a),
      metric_age_a = as.numeric(lubridate::ymd("2013-04-30") - dob_a) / 365,
      metric_age_b = as.numeric(lubridate::ymd("2017-03-31") - dob_b) / 365,
      gender_code = paste0(pmin(gender_code_a, gender_code_b), pmax(gender_code_a, gender_code_b)),
      metric_gender_code_ff = ifelse(grepl("f", gender_code), 1, 0),
      metric_gender_code_mm = ifelse(grepl("m", gender_code), 1, 0),
      race_code = paste0(pmin(race_code_a, race_code_b), pmax(race_code_a, race_code_b)),
      metric_race_code_same = race_code_a == race_code_b,
      metric_race_code_ww_bb = race_code %in% c("ww", "bb"),
      metric_potential_marriage = (!is.na(metric_age_a) | !is.na(metric_age_b)) &
        ((metric_age_a >= 20) | (metric_age_b >= 20)) &
        (!is.na(lname_a) & !is.na(lname_b)) &
        (lname_a != lname_b) &
        (gender_code_a == "f" | gender_code_b == "f"),
      metric_name_swaps = (!is.na(fname_a) & !is.na(fname_b) & !is.na(lname_a) & !is.na(lname_b)) &
        (lname_a == fname_b & lname_b == fname_a),
  ...
  message("🚩 Compute string similarity features...")
  safe_string_metrics <- purrr::safely(summarise_all_string_metrics)
  
  df_fname_metrics <- purrr::map2_dfr(
    df_res$fname_a, df_res$fname_b,
    ~ safe_string_metrics(.x, .y, "fname")$result %||% tibble()
  )
  df_lname_metrics <- purrr::map2_dfr(
    df_res$lname_a, df_res$lname_b,
    ~ safe_string_metrics(.x, .y, "lname")$result %||% tibble()
  )
  
  message("🚩 Merge and return final features...")
  df_res_final <- bind_cols(df_res, df_fname_metrics, df_lname_metrics) %>%
    select(pair_id, fname_a, fname_b, lname_a, lname_b,
           starts_with("dob"), starts_with("gender_code"), starts_with("race_code"),
           starts_with("metric"), everything()) %>%
    mutate(match = factor(match, levels = c("unmatch", "match"))) %>%
    as.data.frame()
...
generate_pairs <- function(df_a_mod, df_b_mod, data_pref) {
  ...
  # Step 0: Check input
  stopifnot("county_desc" %in% colnames(df_a_mod))
  stopifnot("county_desc" %in% colnames(df_b_mod))
  
  # Set output path
  data_path <- "data/paper2/"
  dir.create("data/paper2", recursive = TRUE, showWarnings = FALSE)
  ...
  df_exact_matches <- df_a_mod %>%
    select(id_a, fname, lname, dob, gender_code, race_code) %>%
    inner_join(df_b_mod %>% select(id_b, fname, lname, dob, gender_code, race_code)) %>%
    select(starts_with("id"))
  write_rds_mod(df_exact_matches, "df_exact_matches")
  
  message("Id matches (same county only)")
  df_vrn_matches <- df_a_mod %>%
    select(id_a, voter_reg_num, county_desc) %>%
    inner_join(df_b_mod %>% select(id_b, voter_reg_num, county_desc),
               by = c("voter_reg_num", "county_desc")) %>%
    select(starts_with("id")) %>%
    mutate(match = "match") %>%
    distinct()
  write_rds_mod(df_vrn_matches, "df_vrn_matches")
  
  message("Unexact Matches (what will finally be used)")
  df_matches_unexact <- df_vrn_matches %>%
    anti_join(df_exact_matches) %>%
    left_join(df_a_mod, by = "id_a") %>%
    left_join(df_b_mod, by = "id_b", suffix = c("_a", "_b")) %>%
    mutate(pair_id = row_number())
  write_rds_mod(df_matches_unexact, "df_matches_unexact")
  
  message("Generating close non matches")
  
  #fname_block_counts <- df_b_mod %>% count(fname) %>% filter(n <= 50) %>% pull(fname)
  #lname_block_counts <- df_b_mod %>% count(lname) %>% filter(n <= 50) %>% pull(lname)
  #dob_block_counts <- df_b_mod %>% count(dob) %>% filter(n <= 50) %>% pull(dob)
  fname_block_counts <- df_b_mod %>% pull(fname)
  lname_block_counts <- df_b_mod %>% pull(lname)
  dob_block_counts   <- df_b_mod %>% pull(dob)
  
  
  df_blocks <- bind_rows(
    df_matches_unexact %>% filter(fname_a %in% fname_block_counts) %>%
      select(id_a, fname_a) %>%
      inner_join(df_b_mod %>% select(id_b, fname), by = c("fname_a" = "fname")) %>%
      select(id_a, id_b),
    
    df_matches_unexact %>% filter(fname_b %in% fname_block_counts) %>%
      select(id_b, fname_b) %>%
      inner_join(df_a_mod %>% select(id_a, fname), by = c("fname_b" = "fname")) %>%
      select(id_a, id_b),
    
    df_matches_unexact %>% filter(lname_a %in% lname_block_counts) %>%
      select(id_a, lname_a) %>%
      inner_join(df_b_mod %>% select(id_b, lname), by = c("lname_a" = "lname")) %>%
      select(id_a, id_b),
    
    df_matches_unexact %>% filter(lname_b %in% lname_block_counts) %>%
      select(id_b, lname_b) %>%
      inner_join(df_a_mod %>% select(id_a, lname), by = c("lname_b" = "lname")) %>%
      select(id_a, id_b),
    
    df_matches_unexact %>% filter(dob_a %in% dob_block_counts) %>%
      select(id_a, dob_a) %>%
      inner_join(df_b_mod %>% select(id_b, dob), by = c("dob_a" = "dob")) %>%
      select(id_a, id_b),
    
    df_matches_unexact %>% filter(dob_b %in% dob_block_counts) %>%
      select(id_b, dob_b) %>%
      inner_join(df_a_mod %>% select(id_a, dob), by = c("dob_b" = "dob")) %>%
      select(id_a, id_b)
  ) %>%
    distinct() %>%
  ...
  df_blocks <- df_blocks %>%
    mutate(
      fname_jw = stringdist(fname_a, fname_b, "jw", p = 0.1),
  ...
  df_thresholds <- df_blocks %>%
    mutate(
      thresh_fname = fname_jw < 0.15,
      thresh_lname = lname_jw < 0.15,
      thresh_dob = (day_match + month_match + swap_match + year_match) >= 2,
  ...
  df_unmatches_unexact <- df_thresholds %>%
    sample_n(nrow(.)) %>%
    ...
    attach_dbs_to_ids_dfs(df_a_mod, df_b_mod)
  write_rds_mod(df_unmatches_unexact, "df_unmatches_unexact")
  
  message("Pairs")
  df_pairs <- df_unmatches_unexact %>%
    bind_rows(df_matches_unexact) %>%
    sample_n(nrow(.))
  write_rds_mod(df_pairs, "df_pairs")
  
  df_pairs %>% as.data.frame()
}
...
predict_nn_2 <- function(model, df_ts, df_tr) {
  ...
  tr_means <- map_dbl(df_tr, ~ mean(.x, na.rm = TRUE))
  tr_sds <- map_dbl(df_tr, ~ sd(.x, na.rm = TRUE))
  ...
  df_ts <- pmap_df(list(a = df_ts, b = tr_means, c = tr_sds), function(a, b, c) {
    (a - b) / c
  }) %>%
    mutate_all(fill_na_0) %>%
    mutate(match = ifelse(match == min(match), 0, 1))
  
  ts_x <- df_ts %>% select(-pair_id, -match) %>% as.matrix()
...
build_model <- function(inp_len) {
  model <- keras_model_sequential() %>%
    layer_dense(units = 64, activation = "relu", input_shape = inp_len) %>%
...
encode_match <- function(x) {
  ifelse(as.character(x) == "match", 1, 0)
}




compute_thresholds <- function(y_true, pred_prob) {
  
  stopifnot(length(y_true) == length(pred_prob))
  
  keep <- !is.na(y_true) & !is.na(pred_prob)
  y_true <- y_true[keep]
  pred_prob <- pred_prob[keep]
  
  if (length(unique(y_true)) < 2) {
    stop("compute_thresholds(): y_true must have two classes")
  }
  ...
  tibble::tibble(
    threshold = thr,
    youden = sens + spec - 1,
    roc_dist = (1 - sens)^2 + (1 - spec)^2,
    sens_spec_diff = abs(sens - spec)
  ) %>%
    summarise(
      youden     = threshold[which.max(youden)],
      minROCdist = threshold[which.min(roc_dist)],
      equsen     = threshold[which.min(sens_spec_diff)]
    )
}
...
evaluate_model <- function(model, df_test, ...) {
  if (!is.data.frame(df_test)) {
    stop("df_test is not a data.frame.")
  }
  
  if (!"match" %in% names(df_test)) {
    stop("`match` column not found in df_test.")
  }
  
  pred_prob <- predict(model, df_test, type = "prob", na.action = na.pass)[, "match"]
  
  message("[evaluate_model] Rows in df_test: ", nrow(df_test))
  message("[evaluate_model] Preview pred_prob: ", paste0(round(head(pred_prob, 3), 3), collapse = ", "))
  
  calculate_metrics(df_test, pred_prob)
}


extract_summary_metrics <- function(metrics) {
  tibble(
    ...
    auroc      = metrics$auroc,
    auprc      = metrics$auprc,
    review_pct_100 = metrics$review_pct_100,
    review_pct_99  = metrics$review_pct_99,
    review_pct_95  = metrics$review_pct_95,
    review_pct_90  = metrics$review_pct_90
  )
}
...
calculate_metrics <- function(df_test, pred_prob) {
  message("[calculate_metrics] Rows received: ", nrow(df_test))
  
  match <- as.vector(df_test$match)
  match <- ifelse(as.character(match) == "match", 1, 0)
  
  if (length(match) != length(pred_prob)) {
    stop(glue("Length mismatch: truth = {length(match)}, pred_prob = {length(pred_prob)}"))
  }
...
calc_review_pct <- function(match, match_prob, ppv = 1, npv = 1){
  ...
  pos_probs <- match_prob[match_prob > 0.6]
  neg_probs <- rev(match_prob[match_prob <= 0.6])
...
# Function to calculate the performance metrics based on predicted probabilities
calculate_metrics_prob <- function(match, pred_prob) {
  match_pred <- ifelse(pred_prob >= 0.6, 1, 0)
  metrics <- list()
  
  metrics$confusion_matrix <- table(
    factor(match_pred, levels = c(0, 1)),
    factor(match, levels = c(0, 1))
  )
  ...
  metrics$accuracy <- MLmetrics::Accuracy(match_pred, match)
  metrics$auc <- pROC::auc(match, pred_prob)
  metrics$precision <- MLmetrics::Precision(match_pred, match)
  metrics$recall <- MLmetrics::Recall(match_pred, match)
  metrics$f1 <- MLmetrics::F1_Score(match_pred, match)
  
  roc_obj <- pROC::roc(match, pred_prob, quiet = TRUE)
  metrics$auroc <- as.numeric(roc_obj$auc)
  

  
  pr_obj <- PRROC::pr.curve(
    scores.class0 = pred_prob[match == 1], 
    scores.class1 = pred_prob[match == 0],
    curve = TRUE
  )
  metrics$auprc <- pr_obj$auc.integral
  
  # Store PR curve for plotting
  metrics$pr_curve <- tibble::tibble(
  ...
  # ======================================================
  numeric_entries <- metrics[sapply(metrics, function(x) is.numeric(x) && length(x) == 1)]
  
  metrics$df_metric_table <- tibble::tibble(
    metric = names(numeric_entries),
    value = unlist(numeric_entries)
  )
 ...
 # metrics$df_metric_table <- metrics %>% 
  #  enframe() %>% 
   # rename(metric = name) %>% 
...
get_bdays <- function(df_a, df_b, year_a, year_b) {
  library(dplyr)
  library(stringr)
  library(lubridate)
  library(glue)
  ...
  df_a_t <- df_a %>%
    select(voter_reg_num, county_desc, first_name, mname, last_name,
           birth_age, gender_code, race_code, res_street_address)
  
  df_b_t <- df_b %>%
    select(voter_reg_num, county_desc, first_name, mname, last_name,
           birth_age, gender_code, race_code, res_street_address)
  
  df_a_minus_b_t <- df_a_t %>%
    anti_join(df_b_t, by = c("voter_reg_num", "county_desc")) %>%
    mutate(as_of = as.Date(glue("{year_a}-01-01")))
  
  df_a_int_b_t <- df_a_t %>%
    semi_join(df_b_t, by = c("voter_reg_num", "county_desc")) %>%
    mutate(as_of = as.Date(glue("{year_a}-01-01")))
  
  df_b_minus_a_t <- df_b_t %>%
    anti_join(df_a_t, by = c("voter_reg_num", "county_desc")) %>%
    mutate(as_of = as.Date(glue("{year_b}-01-01")))
  
  df_t <- bind_rows(df_a_minus_b_t, df_a_int_b_t, df_b_minus_a_t) %>%
    mutate(
      dob = map2_dbl(as_of, birth_age, gen_birthday_from_age_scr) %>% as_date()
    ) %>%
    select(voter_reg_num, county_desc, last_name, res_street_address, birth_age, dob)
  ...
  df_with_twins <- df_t %>%
    ...
    left_join(
      df_t %>%
        ...
        sample_n(1) %>%
        mutate(twin_id = cur_group_id(), bday_twin = dob) %>%
        select(last_name, res_street_address, birth_age, bday_twin, twin_id),
      by = c("last_name", "res_street_address", "birth_age")
    ) %>%
    distinct(voter_reg_num, county_desc, .keep_all = TRUE) %>%
    select(voter_reg_num, county_desc, dob, bday_twin, twin_id)
  
  return(df_with_twins)
}
...
build_nn <- function(df_tr, path_nn, epochs = 50) {
  ...
  df_tr <- df_tr %>%
    select(-pair_id) %>%  # 保留所有 selected_features，除了 pair_id
    mutate_if(is.logical, as.integer) %>%
    mutate_if(is.factor, as.integer) %>%
    mutate_if(is.integer, as.double)
  ...
  df_tr <- pmap_df(list(a = df_tr, b = tr_means, c = tr_sds), function(a, b, c) {
    (a - b) / c
  }) %>%
    mutate_all(~replace(., is.na(.), 0)) %>%
    mutate(match = ifelse(match == min(match), 0, 1))
  
  tr_x <- df_tr %>% select(-match) %>% as.matrix()
  tr_y <- df_tr$match
  
  model <- keras_model_sequential()
  model$add(layer_dense(units = 64, activation = "relu", input_shape = c(ncol(tr_x))))
  model$add(layer_batch_normalization())
  model$add(layer_dropout(rate = 0.001))
